...
- **保存原始包 (Save raw packets)**：在设置中可以启用“保存原始包”选项。启用后，捕获到的每个数据包会同时保存其原始字节流（Base64 编码）和精确时间戳，以保证后续导出为 PCAP 时能完整还原原始流量。
- **存储影响**：保存原始字节会显著增加存储需求（视网络流量和包大小而定）。建议在仅用于教学演示或需要精确回放时启用；长时间运行或高吞吐场景下请保持此选项关闭以节省磁盘空间。
- **导出为 PCAP**：当原始包存在时，导出功能会优先使用原始字节并保留捕获时间戳，生成兼容 Wireshark 的 PCAP 文件；如果原始字节不可用，系统会退回到基于解析字段的重构方法（可能丢失部分原始负载或帧层元数据）。
- **位置与操作**：打开 `⚙️ 设置`，切换 `保存原始包`；在捕获页面使用“💾 保存捕获”或新增加的“📥 导出为 PCAP”按钮完成导出。
